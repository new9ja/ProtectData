...
<a name="faq0"></a>
**(0) How do I use ProtectData?**

* Enable the ProtectData firewall using the switch in ProtectData's action bar
* Allow (blueish\*) or deny (reddish\*) Wi-Fi or mobile internet access using the icons next to an application name in ProtectData's applications list

You can use *Settings > Defaults* to change from block/blacklist mode (disable *Block Wi-Fi* and *Block mobile*, and then block unwanted applications in ProtectData's applications list) to allow/whitelist mode (enable *Block Wi-Fi* and *Block mobile*, and then allow desired applications in ProtectData's applications list).

\* Depending on the theme you use, the icons may be:
* Allowed (internet access permitted): reen/ blue / purple / grayblue / peach / greenish
* Blocked (internet access denied): blood red/  salmon / brown / light gold / beige / teal

<a name="faq1"></a>
**(1) Can ProtectData completely protect my privacy?**

No - nothing can completely protect your privacy.
ProtectData will do its best, but it is limited by the fact it must use the Android VPN service.
...
However, this does not work properly on all Android versions/variants causing ProtectData to hang and block all connections.

Android N and later allows ProtectData to be an [Always-On VPN](https://developer.android.com/preview/features/afw.html#always-on-vpn).
...
<a name="faq2"></a>
**(2)  Can I use another VPN application while using ProtectData**

If the VPN application is using the [VPN service](http://developer.android.com/reference/android/net/VpnService.html),
then no, because ProtectData needs to use this service. Android allows only one application at a time to use this service.

ProtectData is a firewall application, so there is no intention to add VPN support.
However, ProtectData supports a [SOCKS5 proxy](https://en.wikipedia.org/wiki/SOCKS) to chain VPN applications.

<a name="faq3"></a>
**(3) Can I use ProtectData on any Android version?**

No, the minimum required Android version is 5.1 (<a href= "https://developer.android.com/about/versions/lollipop">Lollipop</a>)

<a name="faq4"></a>
**(4) Will ProtectData use extra battery power?**

By default ProtectData will hardly use any battery power.
All settings resulting in extra battery usage, like IP filtering and logging, have a warning.
If ProtectData uses a lot of battery power, please double check your settings.
...
Note that Android often incorrectly contribute battery usage of other apps to ProtectData,
because the network traffic of other apps is flowing through ProtectData.
This means that it might look like ProtectData is using a lot of battery power,
but that in fact the total battery usage of all apps is still the same.

<a name="faq6"></a>
**(6) Will ProtectData send my internet traffic to an external (VPN) server?**
...
The [Android VPN service](http://developer.android.com/reference/android/net/VpnService.html) is being used to locally route all internet traffic to ProtectData so no root is required to build this firewall application.
ProtectData, unlike all other no-root firewalls applications, is 100% open source, so when you are in doubt you can check [the source code](https://github.com/M66B/ProtectData/) yourself.
...
By showing all applications, ProtectData allows you to control internet access even *before* such an update occurs.

<a name="faq8"></a>
**(8) What do I need to enable for the Google Play™ store app to work?**

You need 3 packages (applications) enabled (use search in ProtectData to find them quickly):
...
Note that ProtectData does *not* require any Google service to be installed.
...
No, because if Tasker is allowed to disable ProtectData, any application can disable ProtectData.
Allowing a security application to be disabled by other applications is not a good idea.

<a name="faq13"></a>
**(13) How can I remove the ongoing ProtectData entry in the notification screen?**

* Long click the ProtectData notification
* Tap the 'i' icon
...
this operation will also disable any information or warning notifications from ProtectData,
...
F-Droid builds are not supported because I have no control over if and when the F-Droid version of ProtectData will be updated,
...
<a name="faq17"></a>
**(17) Why is ProtectData using so much memory?**

It isn't.  ProtectData doesn't allocate any memory, except a little for displaying the user interface elements and for buffering traffic.
...
and is incorrectly attributed to ProtectData instead to the Google Play™ store app.

<a name="faq18"></a>
**(18) Why can't I find ProtectData in the Google Play™ store app?**

ProtectData requires at least Android 5.1, so it is not available in the Google Play™ store app on devices running prior Android versions.
...
You can block system applications and components, like Google Play services, by enabling the advanced ProtectData option *Manage system apps*.
...
To be sure, you can disable battery optimizations for ProtectData manually like this:

```
Android settings > Battery > three dot menu > Battery optimizations > Dropdown > All apps > ProtectData > Don't optimize > Done
```

The procedure to accomplish this can vary between devices.

Disabling doze mode for ProtectData cannot be done from within ProtectData
because, according to Google, ProtectData is [not an application type allowed to do this](http://developer.android.com/training/monitoring-device-state/doze-standby.html#whitelisting-cases).

<a name="faq22"></a>
**(22) Can I tether (use the Android hotspot) / use Wi-Fi calling while using ProtectData?**

Yes, but you'll need to enable subnet routing and tethering in the ProtectData network settings.
...
which is shown by Android and not by ProtectData, unfortunately, cannot be removed.
...
To prevent a bad user experience, ProtectData includes a predefined rule to default allow the Google connectivity services.

You can find all predefined rules [here](https://github.com/new9ja/ProtectData/blob/master/app/src/main/res/xml/predefined.xml).
...
You can only purchase pro features when you have installed ProtectData from the Google Play store.

<a name="faq30"></a>
**(30) Can I also run AFWall+ on the same device?**

Unless you are just testing ProtectData, there is no current reason to use them both, since they cover the same function (firewall),
although with different base needs (AFWall+ needs a rooted device) and ways of doing their thing (AFWall+ uses iptables whereas ProtectData uses a VPN).

Also you need to keep per application access rules _always_ in sync between AFWall+ and ProtectData,
...
Some pointers on how to set up AFWall+ to be used simultaneously with ProtectData:
* if not using filtering in ProtectData, applications _need_ direct internet access (Wi-Fi and/or mobile) in AFWall+
* if using filtering, ProtectData will _need_ internet access (Wi-Fi and/or mobile) in AFWall+
* if using filtering, when you un/reinstall ProtectData, remember to re-allow ProtectData in AFWall+
* if using filtering, applications _need_ VPN internet access (check the box to show that option in AFWall+ settings)

This question was community contributed. There is no support on using ProtectData and AFWall+ together.
...
<a name="faq32"></a>
**(32) Why is the battery/network usage of ProtectData so high?**

This is because Android counts battery and network usage which is normally counted for other applications
against ProtectData in IP filtering mode. The total battery usage is slightly higher when IP filtering mode is enabled.
...
ProtectData "asks" Android to start the local VPN service,
but some Android versions contain a bug which prevents the VPN from starting (automatically).
Sometimes this is caused by updating ProtectData.
Unfortunately this cannot be fixed by ProtectData.
You can try to restart your device and/or revoke the VPN permissions from ProtectData using the Android settings.
Sometimes it helps to uninstall and install ProtectData again (be sure to export your settings first!).
...
Despite ProtectData being *really* a lot of work, only some of the convenience and advanced features need to be purchased,
which means that ProtectData is basically free to use
and that you don't need to pay anything to reduce your data usage, increase battery life, and increase your privacy.

Also note that most free applications will appear not to be sustainable in the end, whereas ProtectData is properly maintained and supported,
...
<a name="faq38"></a>
**(38) Why did ProtectData stop running?**

On most devices, ProtectData will keep running in the background with its foreground service.
On some devices (in particular some Samsung models), where there are lots of applications competing for memory, Android may still stop ProtectData as a last resort.
...
Unfortunately this cannot be fixed by ProtectData, and can be considered a shortcoming of the device and/or as a bug in Android.
...
You can workaround this problem by enabling the watchdog in the ProtectData advanced options to check every 10-15 minutes.
...
Since ProtectData blocks, unlike any other no-root firewall, domain names instead of IP addresses there is hardly a need for wildcards.
...
Access to domain names shown in the application access log (drill down in the ProtectData application settings) will not be notified again,
...
<a name="faq45"></a>
**(45) Does ProtectData handle incoming connections?**
...
If a purchased pro feature doesn't work [as described](https://protectdatang.blogspot.com)
...
ProtectData blocks traffic based on the IP addresses an application is trying to connect to.
...
This could result in ProtectData too early or too late purging a DNS record from its own cache,
resulting in not recognizing an IP address or recognizing a wrong IP address.
You can try to workaround this by changing the DNS TTL value setting of ProtectData.
This value is used as a minimum DNS TTL value in an attempt to mimick the behavior of Android.

ProtectData will also block traffic while restarting the Android VPN service to apply new rules,
for example when connectivity changes or when the screen is turned on or off.

<a name="faq49"></a>
**(49) Does ProtectData encrypt my internet traffic / hide my IP address?**

ProtectData is a firewall application that filters internet traffic on your device (see also [this question](#user-content-faq6)),
so it is not meant to - and does not - encrypt your internet traffic or hide your IP address.

<a name="faq50"></a>
**(50) Will ProtectData automatically start on boot?**

Yes, ProtectData will automatically be started on boot if you powered off your device with ProtectData enabled and ProtectData is not installed on external storage.
...
<a name="faq51"></a>
**(51) Why does ProtectData block all internet traffic?!**

Make sure you have put ProtectData on the doze exception list (Android 6 Marshmallow or later)
and that Android allows ProtectData to use the internet in the background (see also [this question](#user-content-faq21)).

Make sure you are not running ProtectData in allow (whitelist) mode (check the ProtectData default settings).
...
Mostly, you can workaround this bug by enabling filtering in ProtectData's *Advanced options*.
If this doesn't solve the issue, the problem can unfortunately not be fixed or worked around by ProtectData.
...
Tor with ProtectData is only supported in the [XDA ProtectData forum](http://forum.xda-developers.com/showthread.php?t=3233012).
There is no personal support on Tor with ProtectData, because I don't use Tor myself.
...
In ProtectData's _Network options_ enable _Subnet routing_ and in _Advanced options_ toggle on
...
<a name="faq55"></a>
**(55) Why does ProtectData connect to Amazon / ipinfo.io / 216.239.34.21?**

ProtectData connects to Amazon / [ipinfo.io](https://ipinfo.io/) to show the names and organizations for IP addresses.
If you don't want this, just disable showing names and organizations using the three dot menu in the global log view.

<a name="faq56"></a>
**(56) Why does ProtectData allow all internet traffic?!**

ProtectData can block each and every application, even system applications and components.

ProtectData, by default, allows all traffic to prevent hard to find problems.  You need to selectively block traffic yourself by tapping on the mobile or Wi-Fi icons.

Be aware that ProtectData will allow traffic to an application when the screen is on and the condition *'when screen on'* is enabled.

<a name="faq57"></a>
**(57) Why does ProtectData use so much data?**

Basically, ProtectData doesn't use data itself.
However, many Android versions incorrectly account data of other applications flowing through ProtectData to ProtectData instead of to the applications.
The data usage of other applications will be zero with ProtectData enabled in this case.

The total data usage of your device will be the same with and without ProtectData.
...
* Open ProtectData and navigate to the pro features screen; ProtectData will check the purchases again
...
* An application like ProtectData cannot select which Google account to use
...
Please see the [compatibility section](https://github.com/new9ja/ProtectData/#compatibility) about this
(you might need to request the desktop version to see this section if you are using a mobile device).

<a name="faq61"></a>
**(61) Help, ProtectData crashed!**

ProtectData rarely crashes ("unexpectedly stopped"), but if it crashed (which is something different than being stopped by Android, see [this FAQ](#user-content-faq38)),
...
or that you are trying to install ProtectData on a not supported Android version.

<a name="faq63"></a>
**(63) Why is all DNS traffic allowed?**

ProtectData blocks unlike any other Android firewall on real domain names.
For this a list of domain names and IP address needs to be built.
For this purpose, ProtectData allows all DNS traffic, even if the domain name is listed in the hosts file.
...
<a name="faq65"></a>
**(65) Why can ProtectData not block itself?**

First of all, if ProtectData could block itself, you should trust that ProtectData really blocks itself,
which is basically the same as trusting that ProtectData doesn't connect to the internet when not needed.

Note that ProtectData needs to connect to the internet to forward traffic of other apps to the internet and to lookup information on IP addresses,
see also [this FAQ](#user-content-faq55).

ProtectData could block itself in older versions,
...
So, because blocking ProtectData with itself didn't added anything useful
and to save on battery power and to prevent crashes blocking ProtectData with itself was removed.
...
However, you can block ads for all apps with ProtectData, please see [here](https://github.com/new9ja/ProtectData/blob/master/ADBLOCKING.md) about how to.
...
**ProtectData is supported for phones and tablets only, so not for other device types like televisions or vehicles.**

**If you didn't find the answer to your question, you can ask your questions [in this forum](http://forum.xda-developers.com/showthread.php?t=3233012) or contact me by using [this contact form](https://protectdatang.blogspot.com/p/contact-us.html)**.
